...
export function createTicket(projectId: string, data: { title: string; description: string }) {
	return apiFetch<{ id: string }>(`/api/v1/projects/${projectId}/tickets`, {
		method: 'POST',
...
export function rerunTicket(projectId: string, ticketId: string) {
	return apiFetch<{ status: string; kind: string }>(`/api/v1/projects/${projectId}/tickets/${ticketId}/rerun`, {
		method: 'POST',
	});
}
