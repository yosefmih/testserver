...
async def create_ticket(request: Request, project_id: str, body: CreateTicketRequest):
    user_id = get_current_user_id(request)
    pool = await get_pool()
    ...
    issue_id = f"manual-{secrets.token_hex(8)}"
    ticket = await pool.fetchrow("""
        INSERT INTO tickets (project_id, linear_issue_id, linear_issue_title, linear_issue_description, status)
        VALUES ($1, $2, $3, $4, 'active')
        RETURNING id
    """, project_id, issue_id, title, body.description)
    ...
    logger.info("Manual ticket %s created with initial run for project %s", ticket_id, project_id)
    return {"id": ticket_id}
...
async def cancel_ticket(request: Request, project_id: str, ticket_id: str):
    ...
    if ticket["volume_id"]:
        try:
            await delete_volume(ticket["volume_id"])
...
async def get_run_logs(request: Request, project_id: str, ticket_id: str, run_id: str):
    ...
    try:
        lines = await get_sandbox_logs(run["sandbox_id"])
        return {"logs": lines}
    except Exception as e:
