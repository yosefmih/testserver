...
8. The review run reuses the same persistent Porter Sandbox volume, so the agent has full context from previous runs
...
GitHub (webhook) ──►      │              Container:
                          ...
                          │              - GitHub MCP server
                          │              - Linear MCP server
                          │              - /workspace (Porter Sandbox volume)
                          │                    │
...
- **Ticket** — one per Linear issue. Tracks the PR URL, Porter Sandbox volume, and lifecycle status (active/merged/closed)
...
- **Persistent volumes**: Each ticket gets an Porter Sandbox volume mounted at `/workspace`. The agent clones into `/workspace/repo` and context is preserved across runs
...
linear-autopilot/
  backend/
    ...
    services/
      ...
      sandbox_runner.py  # Sandbox creation with Porter Sandbox volumes, callback tokens, prompt building
...
- **tickets** — one per Linear issue (PR metadata, Porter Sandbox volume ID, status, debounce timer)
